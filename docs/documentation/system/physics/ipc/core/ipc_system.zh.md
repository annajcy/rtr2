...
`body.precompute()` 现在会从每个 body 的 material object 里读取 density，因此 `mass_diag` 的来源是 `TetBody::material`，而不是 `TetBody` 上单独的标量字段。
...
const TetBody& tet_body(std::size_t index) const;           // 按索引取 body
