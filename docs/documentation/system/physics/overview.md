...
2. **`PhysicsWorld` (Core Physics Sandbox)**
   ...
   - **Core Pipeline (`tick`)**: In each frame, it first integrates dynamic rigid bodies, then builds a single contact snapshot for that frame, runs a velocity-phase PGS solve over `velocity_iterations`, and finally performs a position-phase penetration correction over `position_iterations`. Contact impulses are accumulated only within the current frame (`normal_impulse_sum` / `tangent_impulse_sum`); there is no cross-frame warm start or persistent manifold cache in this version.
