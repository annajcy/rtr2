...
public:
    ...
    glm::vec3 position() const { return local_position(); }
    glm::quat rotation() const { return local_rotation(); }
    glm::vec3 scale() const { return local_scale(); }
...
inline bool ConstNodeView::valid() const {
    return m_graph != nullptr && m_graph->has_node(m_id);
}

inline GameObjectId ConstNodeView::parent_id() const {
    return m_graph->checked_record(m_id).parent_id;
}

inline const std::vector<GameObjectId>& ConstNodeView::children() const {
    return m_graph->checked_record(m_id).children;
}

inline const glm::vec3& ConstNodeView::local_position() const {
    return m_graph->checked_record(m_id).local_position;
}

inline const glm::quat& ConstNodeView::local_rotation() const {
    return m_graph->checked_record(m_id).local_rotation;
}

inline const glm::vec3& ConstNodeView::local_scale() const {
    return m_graph->checked_record(m_id).local_scale;
}

inline const glm::mat4& ConstNodeView::world_matrix() const {
    return m_graph->checked_record(m_id).world_matrix;
}

inline glm::vec3 ConstNodeView::world_position() const {
    return SceneGraph::extract_world_position(world_matrix());
}

inline glm::quat ConstNodeView::world_rotation() const {
    return SceneGraph::extract_world_rotation(world_matrix());
}

inline glm::vec3 ConstNodeView::world_scale() const {
    return SceneGraph::extract_world_scale(world_matrix());
}

inline bool ConstNodeView::dirty() const {
    return m_graph->checked_record(m_id).dirty;
}

inline bool ConstNodeView::self_enabled() const {
    return m_graph->checked_record(m_id).self_enabled;
}

inline bool ConstNodeView::hierarchy_active() const {
    return m_graph->checked_record(m_id).hierarchy_active;
}

inline void NodeView::set_local_position(const glm::vec3& value) {
...
inline void NodeView::set_local_rotation(const glm::quat& value) {
    auto& record = const_cast<SceneGraph*>(m_graph)->checked_record(m_id);
    record.local_rotation = value;
    const_cast<SceneGraph*>(m_graph)->mark_subtree_dirty_recursive(m_id);
}

inline void NodeView::set_local_scale(const glm::vec3& value) {
    auto& record = const_cast<SceneGraph*>(m_graph)->checked_record(m_id);
    record.local_scale = value;
    const_cast<SceneGraph*>(m_graph)->mark_subtree_dirty_recursive(m_id);
}

inline void NodeView::set_local_model_matrix(const glm::mat4& local_model_matrix) {
    glm::vec3 scale = {
...
inline void NodeView::set_world_position(const glm::vec3& value) {
    const_cast<SceneGraph*>(m_graph)->set_world_position_internal(m_id, value);
}

inline void NodeView::set_world_rotation(const glm::quat& value) {
    const_cast<SceneGraph*>(m_graph)->set_world_rotation_internal(m_id, value);
}

inline void NodeView::set_world_scale(const glm::vec3& value) {
    const_cast<SceneGraph*>(m_graph)->set_world_scale_internal(m_id, value);
}

inline glm::vec3 NodeView::rotation_euler() const {
    return glm::degrees(glm::eulerAngles(local_rotation()));
}

inline glm::vec3 NodeView::up() const { return local_rotation() * glm::vec3(0.0f, 1.0f, 0.0f); }
inline glm::vec3 NodeView::down() const { return local_rotation() * glm::vec3(0.0f, -1.0f, 0.0f); }
inline glm::vec3 NodeView::right() const { return local_rotation() * glm::vec3(1.0f, 0.0f, 0.0f); }
inline glm::vec3 NodeView::left() const { return local_rotation() * glm::vec3(-1.0f, 0.0f, 0.0f); }
inline glm::vec3 NodeView::front() const { return local_rotation() * glm::vec3(0.0f, 0.0f, 1.0f); }
inline glm::vec3 NodeView::back() const { return local_rotation() * glm::vec3(0.0f, 0.0f, -1.0f); }

inline glm::vec3 NodeView::world_up() const { return world_rotation() * glm::vec3(0.0f, 1.0f, 0.0f); }
inline glm::vec3 NodeView::world_down() const { return world_rotation() * glm::vec3(0.0f, -1.0f, 0.0f); }
inline glm::vec3 NodeView::world_right() const { return world_rotation() * glm::vec3(1.0f, 0.0f, 0.0f); }
inline glm::vec3 NodeView::world_left() const { return world_rotation() * glm::vec3(-1.0f, 0.0f, 0.0f); }
inline glm::vec3 NodeView::world_front() const { return world_rotation() * glm::vec3(0.0f, 0.0f, 1.0f); }
inline glm::vec3 NodeView::world_back() const { return world_rotation() * glm::vec3(0.0f, 0.0f, -1.0f); }

inline glm::mat4 NodeView::normal_matrix() const {
    return glm::transpose(glm::inverse(world_matrix()));
}

inline void NodeView::look_at_direction(const glm::vec3& target_direction) {
...
inline void NodeView::look_at_point(const glm::vec3& target_point) {
    look_at_direction(target_point - position());
}

inline void NodeView::translate(const glm::vec3& direction, float distance) {
    set_local_position(position() + direction * distance);
}

inline void NodeView::rotate(float angle, const glm::vec3& axis) {
