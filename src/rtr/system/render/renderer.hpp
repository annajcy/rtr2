...
public:
    ...
    private:
        ...
        ComputeJob(rhi::Device& device, rhi::CommandBuffer&& command_buffer, vk::raii::Fence&& fence,
...
public:
    Renderer(int width, int height, std::string title)
        : m_window(width, height, title),
          ...
          m_compute_command_pool(
              m_device, vk::CommandPoolCreateFlagBits::eTransient | vk::CommandPoolCreateFlagBits::eResetCommandBuffer),
    ...
    PipelineRuntime build_pipeline_runtime() {
        return PipelineRuntime{.device          = m_device,
                               .context         = m_context,
                               .window          = m_window,
    ...
    ComputeJob compute_async(const ComputeRecordCallback& record, ComputeCompleteCallback on_complete = {}) {
        ...
        command_buffer.record([&](rhi::CommandBuffer& cb) { record(cb); },
...
private:
    ...
    void transition_swapchain_to_present(const vk::raii::CommandBuffer& command_buffer,
