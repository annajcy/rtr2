...
public:
    ...
    void execute(render::FrameContext& ctx) override {
        if (m_render_pass_resources.uniform_buffer == nullptr ||
            m_render_pass_resources.offscreen_image == nullptr ||
            m_render_pass_resources.offscreen_layout == nullptr ||
            m_render_pass_resources.compute_set == nullptr) {
...
public:
    ...
    void execute(render::FrameContext& ctx) override {
        if (m_render_pass_resources.offscreen_image == nullptr ||
            m_render_pass_resources.offscreen_layout == nullptr ||
            m_render_pass_resources.depth_image == nullptr ||
            m_render_pass_resources.present_set == nullptr) {
...
public:
    ShaderToyPipeline(
        ...
        compute_layout_builder
            .add_binding(0, vk::DescriptorType::eUniformBuffer, vk::ShaderStageFlagBits::eCompute)
            .add_binding(1, vk::DescriptorType::eStorageImage, vk::ShaderStageFlagBits::eCompute);
        m_compute_layout = std::make_unique<rhi::DescriptorSetLayout>(
            compute_layout_builder.build(m_device)
        );

        rhi::DescriptorSetLayout::Builder present_layout_builder;
        present_layout_builder.add_binding(
            0,
            vk::DescriptorType::eCombinedImageSampler,
            vk::ShaderStageFlagBits::eFragment
        );
        m_present_layout = std::make_unique<rhi::DescriptorSetLayout>(
            present_layout_builder.build(m_device)
        );

        rhi::DescriptorPool::Builder descriptor_pool_builder;
        descriptor_pool_builder
            .add_layout(*m_compute_layout, m_frame_count)
    ...
    void render(FrameContext& ctx) override {
        ...
        m_compute_pass->bind_render_pass_resources(ComputePass::RenderPassResources{
        ...
        m_present_pass->bind_render_pass_resources(PresentImagePass::RenderPassResources{
            .offscreen_image = offscreen_frame.image.get(),
            .offscreen_layout = &offscreen_frame.layout,
...
private:
    ...
    void create_present_graphics_pipeline() {
    ...
    void refresh_descriptor_sets() {
        ...
        for (uint32_t index = 0; index < m_frame_count; ++index) {
            rhi::DescriptorWriter compute_writer;
            compute_writer.write_buffer(0, *m_uniform_buffers[index]->buffer(), 0, m_uniform_buffer_size);
            compute_writer.write_storage_image(
                1,
                *m_offscreen_frame_resources[index].image->image_view(),
                vk::ImageLayout::eGeneral
            );
            compute_writer.update(m_device, *m_compute_sets[index]);

            rhi::DescriptorWriter present_writer;
            present_writer.write_combined_image(
                0,
                *m_offscreen_frame_resources[index].image->image_view(),
                *m_offscreen_sampler->sampler(),
                vk::ImageLayout::eShaderReadOnlyOptimal
            );
            present_writer.update(m_device, *m_present_sets[index]);
        }
    }
};
