...
public:
    ForwardPipeline(const render::PipelineRuntime& runtime, const ForwardPipelineConfig& config = {})
        ...
        per_object_layout_builder.add_binding(0, vk::DescriptorType::eUniformBuffer,
                                              vk::ShaderStageFlagBits::eVertex | vk::ShaderStageFlagBits::eFragment);
        m_per_object_layout = std::make_unique<rhi::DescriptorSetLayout>(per_object_layout_builder.build(m_device));

        rhi::DescriptorPool::Builder descriptor_pool_builder;
        descriptor_pool_builder.add_layout(*m_per_object_layout, kMaxRenderables * m_frame_count)
            .set_flags(vk::DescriptorPoolCreateFlagBits::eFreeDescriptorSet);
        m_descriptor_pool = std::make_unique<rhi::DescriptorPool>(descriptor_pool_builder.build(m_device));
    ...
    void set_scene_view(ForwardSceneView scene_view) { m_scene_view = std::move(scene_view); }
    ...
    FrameColorSourceView frame_color_source_view(uint32_t frame_index) const override {
        if (frame_index >= m_color_images.size() || frame_index >= m_color_image_layouts.size() ||
        ...
        return FrameColorSourceView{
            ...
            .extent       = vk::Extent2D{m_color_images[frame_index]->width(), m_color_images[frame_index]->height()}};
    }
    ...
    void render(FrameContext& ctx) override {
        ...
        for (std::size_t i = 0; i < scene_view.renderables.size(); ++i) {
            ...
            ubo.base_color = {
                static_cast<float>(renderable.base_color.x()), static_cast<float>(renderable.base_color.y()),
                static_cast<float>(renderable.base_color.z()), static_cast<float>(renderable.base_color.w())};

            std::memcpy(frame_uniform_buffers[i]->mapped_data(), &ubo, sizeof(ubo));

            draw_items.emplace_back(ForwardPass::DrawItem{.mesh = &mesh, .per_object_set = &frame_sets[i]});
        }
...
private:
    rhi::Mesh& require_mesh(resource::MeshHandle mesh_handle) {
        ...
        if (m_resource_manager == nullptr) {
            logger->error("ForwardPipeline missing resource manager while requesting mesh handle={}",
                          mesh_handle.value);
    ...
    void ensure_scene_targets(vk::Extent2D fallback_extent) {
        ...
        const bool need_recreate = m_scene_extent_dirty || m_scene_target_extent.width != desired_extent.width ||
                                   m_scene_target_extent.height != desired_extent.height || m_color_images.empty() ||
    ...
    void create_per_object_resources() {
        ...
        for (uint32_t frame = 0; frame < m_frame_count; ++frame) {
            ...
            for (uint32_t slot = 0; slot < kMaxRenderables; ++slot) {
                auto buffer = std::make_unique<rhi::Buffer>(rhi::Buffer::create_host_visible_buffer(
            ...
            for (uint32_t slot = 0; slot < kMaxRenderables; ++slot) {
                ...
                writer.write_buffer(0, *frame_buffers[slot]->buffer(), 0, m_uniform_buffer_size);
    ...
    void create_graphics_pipeline() {
        std::vector<vk::PipelineShaderStageCreateInfo> shader_stage_infos = {
            m_vertex_shader_module->stage_create_info(), m_fragment_shader_module->stage_create_info()};
        ...
        vk::StructureChain<vk::GraphicsPipelineCreateInfo, vk::PipelineRenderingCreateInfo> pipeline_info_chain{
            graphics_pipeline_create_info, pipeline_rendering_info};

        m_pipeline =
            vk::raii::Pipeline{m_device->device(), nullptr, pipeline_info_chain.get<vk::GraphicsPipelineCreateInfo>()};
    }
};
