...
inline WorldCollider to_world_collider(const FromType& collider_shape,
                                       const PhysicsTransform& local_transform,
                                       const PhysicsTransform& world_transform);

template <>
inline WorldCollider to_world_collider(const BoxShape& box_shape,
                                       const PhysicsTransform& local_transform,
                                       const PhysicsTransform& world_transform) {
    const pbpt::math::Vec3 scaled_local_pos = local_transform.position * world_transform.scale;
    const pbpt::math::Vec3 center = world_transform.position + (world_transform.rotation * scaled_local_pos);
    const pbpt::math::Quat rotation = pbpt::math::normalize(world_transform.rotation * local_transform.rotation);
    const pbpt::math::Vec3 half_extents = box_shape.half_extents * local_transform.scale * world_transform.scale;
...
template <>
inline WorldCollider to_world_collider(const SphereShape& sphere_shape,
                                       const PhysicsTransform& local_transform,
                                       const PhysicsTransform& world_transform) {
    const pbpt::math::Vec3 scaled_local_pos = local_transform.position * world_transform.scale;
    const pbpt::math::Vec3 center = world_transform.position + (world_transform.rotation * scaled_local_pos);

    const pbpt::math::Vec3 scale = local_transform.scale * world_transform.scale;
    const pbpt::math::Float max_scale = scale.abs().max();
...
template <>
inline WorldCollider to_world_collider(const PlaneShape& plane_shape,
                                       const PhysicsTransform& local_transform,
                                       const PhysicsTransform& world_transform) {
    const pbpt::math::Vec3 scaled_local_pos = local_transform.position * world_transform.scale;
...
template <>
inline WorldCollider to_world_collider(const MeshShape& mesh_shape,
                                       const PhysicsTransform& local_transform,
                                       const PhysicsTransform& world_transform) {
