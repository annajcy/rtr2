...
public:
    ...
    const system::input::InputSystem& input_system() const {
        return *m_input;
    }

    system::render::IRenderPipeline* pipeline() {
        return m_renderer->pipeline();
    }

    const system::render::IRenderPipeline* pipeline() const {
        return m_renderer->pipeline();
    }

    void set_callbacks(RuntimeCallbacks callbacks) {
