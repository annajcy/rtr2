...
private:
    ...
    bool has_registered_body() const {
        return m_registered && m_physics_world.has_rigid_body(m_rigid_body_id);
    }

public:
    RigidBodyComponent(core::GameObject& owner, system::physics::PhysicsWorld& world, pbpt::math::Vec3 velocity)
        : Component(owner),
    ...
    void on_awake() override {
        ...
        state.position = owner().node().world_position();
        state.linear_velocity = m_velocity;
        m_registered = true;
    }
    ...
    pbpt::math::Vec3 position() const {
        ...
        return m_physics_world.get_rigid_body(m_rigid_body_id).state().position;
    }
    ...
    void set_position(const pbpt::math::Vec3& position) {
        ...
        m_physics_world.get_rigid_body(m_rigid_body_id).state().position = position;
    }
    ...
    pbpt::math::Vec3 linear_velocity() const {
        ...
        return m_physics_world.get_rigid_body(m_rigid_body_id).state().linear_velocity;
    }
    ...
    void set_linear_velocity(const pbpt::math::Vec3& linear_velocity) {
        ...
        m_physics_world.get_rigid_body(m_rigid_body_id).state().linear_velocity = linear_velocity;
    }
};
