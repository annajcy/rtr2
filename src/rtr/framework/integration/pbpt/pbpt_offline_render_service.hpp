...
namespace detail {
inline std::shared_ptr<spdlog::logger> offline_service_logger() {
    return rtr::utils::get_logger("framework.integration.pbpt.offline_service");
}
} // namespace detail

inline PbptOfflineRenderService::PbptOfflineRenderService(RenderBackend backend)
    ...
    set_message("Idle.");
    detail::offline_service_logger()->info("PbptOfflineRenderService initialized.");
}

inline PbptOfflineRenderService::~PbptOfflineRenderService() {
    request_cancel();
...
inline bool PbptOfflineRenderService::start(
    const core::Scene& scene,
    resource::ResourceManager& resources,
    const OfflineRenderConfig& config
) {
    auto log = detail::offline_service_logger();
...
inline void PbptOfflineRenderService::request_cancel() {
    m_cancel_requested.store(true);
    if (is_running()) {
        set_message("Cancel requested.");
        detail::offline_service_logger()->warn("Offline render cancel requested.");
    } else {
        detail::offline_service_logger()->info("Offline render cancel requested while not running.");
    }
}

inline OfflineRenderState PbptOfflineRenderService::state() const {
    return m_state.load();
}

inline float PbptOfflineRenderService::progress_01() const {
    return m_progress_01.load();
}

inline const std::string& PbptOfflineRenderService::last_message() const {
...
inline bool PbptOfflineRenderService::is_running() const {
    return state() == OfflineRenderState::Running;
}

inline void PbptOfflineRenderService::run_worker(const OfflineRenderConfig& config) {
    auto log = detail::offline_service_logger();
    try {
        m_backend(
            config,
...
inline void PbptOfflineRenderService::set_message(std::string message) {
    std::scoped_lock lock(m_message_mutex);
