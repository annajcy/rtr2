...
inline bool can_map_shape_to_rtr(
    const pbpt::scene::ShapeInstanceRecord<float>& shape,
...
inline PbptImportPackage import_pbpt_scene_to_scene_with_compatible(
    ...
    PbptImportPackage package{
        ...
        .compatible_info = PbptCompatibleInfo{
            ...
            .passthrough_integrator = pbpt_scene_result.integrator,
    ...
    std::visit(
        [&](const auto& integrator) {
            ...
            if constexpr (std::is_same_v<IntegratorT, pbpt::integrator::PathIntegrator<float, 4>>) {
                ...
                package.result.integrator = PbptIntegratorRecord{
                    .type = "path",
                    .max_depth = max_depth
                };
    ...
    std::visit(
        [&](const auto& camera_any) {
            ...
            if constexpr (std::is_same_v<CameraT, pbpt::camera::ThinLensPerspectiveCamera<float>>) {
                ...
                std::string camera_name = compat_import_detail::make_unique_name(
                    "pbpt_camera",
                    package.result.imported_game_object_id_by_name
                );
                ...
                camera.fov_degrees() = sensor.fov_degrees;
                camera.set_aspect_ratio(static_cast<float>(sensor.film_width) / static_cast<float>(std::max(1, sensor.film_height)));
                camera_go.node().set_local_model_matrix(sensor.to_world);
                (void)scene.set_active_camera(camera_go.id());
                detail::register_imported_game_object(package.result, camera_go.name(), camera_go.id());
                if (options.free_look_input_state != nullptr) {
                    (void)camera_go.add_component<component::FreeLookCameraController>(
                        options.free_look_input_state,
                        &scene.camera_manager()
    ...
    for (const auto& shape : pbpt_scene_result.scene.resources.shape_instances) {
        ...
        const auto material_id = pbpt_scene_result.scene.resources.any_material_library.name_to_id().at(shape.material_ref_name);
        ...
        object_name = compat_import_detail::make_unique_name(
            object_name,
            package.result.imported_game_object_id_by_name
        );
        ...
        (void)go.add_component<component::MeshRenderer>(
            mesh_handle,
            pbpt::math::vec4{reflectance.r, reflectance.g, reflectance.b, 1.0f}
        );
        (void)go.add_component<component::PbptMesh>();
        ...
        detail::register_imported_game_object(package.result, go.name(), go.id());
    ...
    scene.scene_graph().update_world_transforms();
    log->info(
        "PBPT import with compatible info completed (mapped_shapes={}, mapped_lights={}, passthrough_shapes={}).",
        package.result.imported_shape_count,
        package.result.imported_light_shape_count,
...
inline PbptImportPackage import_pbpt_scene_xml_to_scene_with_compatible(
