...
inline PbptImportPackage import_pbpt_scene_to_scene_with_compatible(
    ...
    PbptImportPackage package{.result          = PbptImportResult{},
    ...
    auto integrator_res = dispatch_impl(ImportIntegratorMapperList{}, pbpt_scene_result.integrator, ctx, package);
    if (!integrator_res.matched && options.require_supported_cbox_subset) {
        throw std::runtime_error("Unsupported PBPT integrator in import.");
    }

    auto camera_res = dispatch_impl(ImportCameraMapperList{}, pbpt_scene_result.scene.camera, ctx, package);
    if (!camera_res.matched && options.require_supported_cbox_subset) {
    ...
    for (const auto& shape_record : pbpt_scene_result.scene.resources.shape_instances) {
        try {
            auto shape_res = dispatch_impl(ImportShapeMapperList{}, shape_record, ctx, package);
