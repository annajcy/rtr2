...
namespace compat_export_detail {
...
inline void apply_compatible_passthrough(const CompatibleInfo&          compatible_info,
                                         ::pbpt::serde::PbptXmlResult<float>& result) {
...
inline ::pbpt::serde::PbptXmlResult<float> build_scene_result(
    const core::Scene& scene, resource::ResourceManager& resources, const CompatibleInfo* compatible_info = nullptr,
    int film_width_override = 0, int film_height_override = 0, int spp_override = 0) {
    auto                              log    = pbpt_export_logger();
    ::pbpt::serde::PbptXmlResult<float> result = compat_export_detail::make_initial_xml_result(compatible_info);

    if (compatible_info != nullptr) {
        compat_export_detail::apply_compatible_passthrough(*compatible_info, result);
    }
    ...
    const auto primitives  = compat_export_detail::build_primitives_from_resources(result.scene.resources);
...
inline void write_scene_result(const ::pbpt::serde::PbptXmlResult<float>& result,
