...
inline void sync_sphere_collider_to_physics(const component::SphereCollider& sphere_collider,
                                            system::physics::rb::RigidBodySystem& rigid_body_system) {
    if (!sphere_collider.has_collider() || !rigid_body_system.has_collider(sphere_collider.collider_id())) {
        return;
    }

    auto& collider                      = rigid_body_system.get_collider(sphere_collider.collider_id());
...
inline void sync_box_collider_to_physics(const component::BoxCollider& box_collider,
                                         system::physics::rb::RigidBodySystem& rigid_body_system) {
    if (!box_collider.has_collider() || !rigid_body_system.has_collider(box_collider.collider_id())) {
        return;
    }

    auto& collider                      = rigid_body_system.get_collider(box_collider.collider_id());
...
inline void sync_plane_collider_to_physics(const component::PlaneCollider& plane_collider,
                                           system::physics::rb::RigidBodySystem& rigid_body_system) {
    if (!plane_collider.has_collider() || !rigid_body_system.has_collider(plane_collider.collider_id())) {
        return;
    }

    auto& collider                      = rigid_body_system.get_collider(plane_collider.collider_id());
...
inline void sync_mesh_collider_to_physics(const component::MeshCollider& mesh_collider,
                                          system::physics::rb::RigidBodySystem& rigid_body_system) {
    if (!mesh_collider.has_collider() || !rigid_body_system.has_collider(mesh_collider.collider_id())) {
        return;
    }
    ...
    auto& collider                    = rigid_body_system.get_collider(mesh_collider.collider_id());
    collider.shape                    = system::physics::rb::MeshShape{.local_vertices = mesh_component->local_vertices()};
...
inline void sync_scene_to_rigid_body(core::Scene& scene, system::physics::rb::RigidBodySystem& rigid_body_system) {
    ...
    for (const auto id : active_nodes) {
        ...
        if (rigid_body_component != nullptr && rigid_body_component->enabled() &&
            rigid_body_system.has_rigid_body(rigid_body_component->rigid_body_id())) {
            auto& body = rigid_body_system.get_rigid_body(rigid_body_component->rigid_body_id());
            body.state().scale = game_object->node().world_scale();
            if (body.type() != system::physics::rb::RigidBodyType::Dynamic) {
...
inline void sync_rigid_body_to_scene(core::Scene& scene, system::physics::rb::RigidBodySystem& rigid_body_system) {
    ...
    for (const auto id : active_nodes) {
        ...
        if (!rigid_body_system.has_rigid_body(rigid_body_component->rigid_body_id())) {
            continue;
        }

        const auto& body = rigid_body_system.get_rigid_body(rigid_body_component->rigid_body_id());
        if (body.type() != system::physics::rb::RigidBodyType::Dynamic) {
            continue;
        }
