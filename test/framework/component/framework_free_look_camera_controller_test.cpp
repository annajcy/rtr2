...
TEST(FrameworkFreeLookCameraControllerTest, MovesWithWASDQE) {
    ...
    expect_vec3_near(scene.scene_graph().node(go.id()).world_position(), glm::vec3(0.0f));
}
...
TEST(FrameworkFreeLookCameraControllerTest, ShiftAppliesSprintMultiplier) {
    ...
    node.set_world_position(glm::vec3(0.0f));
...
TEST(FrameworkFreeLookCameraControllerTest, RightMouseRequiredForLook) {
    ...
    core::FrameTickContext ctx{.delta_seconds = 1.0, .unscaled_delta_seconds = 1.0, .frame_index = 0};
    const glm::vec3 before_front = scene.scene_graph().node(go.id()).world_front();

    input.update_mouse_position(30.0, 0.0);
    scene.tick(ctx);
    const glm::vec3 without_right_front = scene.scene_graph().node(go.id()).world_front();
    ...
    scene.tick(ctx);
    const glm::vec3 with_right_front = scene.scene_graph().node(go.id()).world_front();

    EXPECT_GT(glm::length(with_right_front - before_front), 1e-4f);
}
...
TEST(FrameworkFreeLookCameraControllerTest, SceneTickRefreshesWorldTransformAfterComponentUpdate) {
    ...
    const glm::vec3 world_pos = scene.scene_graph().node(go.id()).world_position();
