...
#include "bunny_cloth_smoke_driver.hpp"
...
int main() {
    ...
    const std::vector<rtr::system::physics::VertexID> kPinnedVertices = {1074, 1674, 1691, 1115};

    try {
        rtr::app::AppRuntime runtime(rtr::app::AppRuntimeConfig{
            .window_width = kWidth,
            .window_height = kHeight,
            .window_title = "RTR Cloth Bunny Smoke Demo",
        });
        ...
        auto& scene = runtime.world().create_scene("cloth_bunny_scene");
        ...
        camera_go.node().set_local_position({0.0f, 1.5f, 8.0f});
        ...
        light_go.node().set_local_position({2.5f, 5.0f, 4.5f});
        auto& point_light = light_go.add_component<rtr::framework::component::light::PointLight>();
        point_light.set_color({1.0f, 0.95f, 0.90f});
        ...
        (void)ground_go.add_component<rtr::framework::component::StaticMeshComponent>(
            runtime.resource_manager(), ground_mesh, pbpt::math::Vec4{0.25f, 0.28f, 0.30f, 1.0f});
        ...
        const auto bunny_mesh = runtime.resource_manager().create_from_relative_path<rtr::resource::DeformableMeshResourceKind>(
            "models/stanford_bunny.obj");
        const auto& bunny_cpu_mesh = runtime.resource_manager().cpu<rtr::resource::DeformableMeshResourceKind>(bunny_mesh);
        for (const auto pinned_vertex : kPinnedVertices) {
            if (pinned_vertex < 0 || static_cast<std::size_t>(pinned_vertex) >= bunny_cpu_mesh.vertices.size()) {
                throw std::runtime_error("Pinned bunny vertex index is invalid for the current mesh asset.");
            }
        }

        auto& bunny_go = scene.create_game_object("cloth_bunny");
        bunny_go.node().set_local_position({0.0f, 0.0f, 0.0f});
        bunny_go.node().set_local_scale({10.0f, 10.0f, 10.0f});
        (void)bunny_go.add_component<rtr::framework::component::DeformableMeshComponent>(
            runtime.resource_manager(), bunny_mesh, pbpt::math::Vec4{0.85f, 0.72f, 0.56f, 1.0f});
        (void)bunny_go.add_component<rtr::framework::component::ClothComponent>(
            runtime.physics_system().cloth_world(),
            kPinnedVertices,
            rtr::system::physics::ClothParams{.default_vertex_mass = 1.0f}
        );
        auto& smoke_driver = bunny_go.add_component<rtr::examples::BunnyClothSmokeDriver>();
        smoke_driver.vertical_amplitude = 0.05f;
        smoke_driver.lateral_amplitude = 0.015f;
        smoke_driver.spatial_frequency = 3.0f;
        smoke_driver.time_speed = 2.0f;
