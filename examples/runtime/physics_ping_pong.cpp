...
int main() {
    try {
        ...
        for (int tick = 0; tick < kTickCount; ++tick) {
            ...
            if (state.translation_state.position.x() > kMaxX) {
                state.translation_state.position.x()        = kMaxX;
                state.translation_state.linear_velocity.x() = -std::abs(state.translation_state.linear_velocity.x());
            } else if (state.translation_state.position.x() < kMinX) {
                state.translation_state.position.x()        = kMinX;
                state.translation_state.linear_velocity.x() = std::abs(state.translation_state.linear_velocity.x());
            }

            scene.scene_graph().node(mover.id()).set_local_position(state.translation_state.position);
            scene.scene_graph().update_world_transforms();

            std::cout << tick << ',' << std::fixed << std::setprecision(4) << state.translation_state.position.x()
                      << ',' << state.translation_state.linear_velocity.x() << '\n';
        }
