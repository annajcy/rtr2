...
RTR2 docs are built as a hybrid site:

- authored pages live under `docs/`
- navigation for source-tree-backed sections is maintained by `scripts/docs/generate_source_tree_nav.py`
- API reference is generated by Doxygen into `docs/api/doxygen/html`
- the final site is built by MkDocs Material with `mkdocs-static-i18n`
...
```bash
uv run python scripts/docs/generate_source_tree_nav.py
uv run mkdocs serve
cmake --build --preset conan-debug --target rtr_docs
```

Documentation best practices in this repo:

- keep bilingual docs paired as `*.md` and `*.zh.md`
- use relative links for docs pages and GitHub blob links for source files
- avoid absolute filesystem paths or site-root links like `/zh/...`, which break on GitHub Pages project sites
- regenerate Doxygen when public headers or API-facing behavior change
- run `uv run mkdocs build --strict` before pushing documentation changes
